...
if (email) {
  ...
  var apiUrl1 = `http://localhost/data_from_registered_people.php?email=${encodeURIComponent(email)}`;
  ...
  fetch(apiUrl1)
    .then(response => {
      if (!response.ok) {
        throw new Error('Network response was not ok for apiUrl1');
      }
    ...
    .then(data => {
      if (data && data[0]) {
        ...
        throw new Error('No data received from apiUrl1');
      }
    })
    .then(response => {
      if (!response.ok) {
        throw new Error('Network response was not ok for apiUrl_for_handle_info');
      }
      return response.json();
    })
    .then(data => {
      if (data && data.result && data.result[0]) {
        ...
        throw new Error('Invalid data structure received from user.info API');
      }
    })
    .then(response => {
      if (!response.ok) {
        throw new Error('Network response was not ok for registered_people.php');
      }
    ...
    .then(response => {
      if (!response.ok) {
        throw new Error('Network response was not ok for user.status API');
      }
      return response.json();
    })
    .then(data => {
      if (data && data.result) {
        ...
        throw new Error('Invalid data structure received from user.status API');
      }
    })
    .then(response => {
      if (!response.ok) {
        throw new Error('Network response was not ok for problems.php');
      }
      ...
      return fetch(url_to_get_data_for_pie_chart)
        .then(response => {
          if (!response.ok) {
            throw new Error('Network response was not ok for pie_chart.php');
          }
          return response.json();
        })
        .then(data => {
        ...
        .then(response => {
          if (!response.ok) {
            throw new Error('Network response was not ok for leaderboard.php');
          }
